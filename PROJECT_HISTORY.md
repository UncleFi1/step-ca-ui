...
Цель: убрать лишний текст из публичной формы восстановления и сохранить безопасное поведение без раскрытия аккаунтов.

Реализовано:

- убрана фраза про SMTP из описания формы восстановления;
- сохранён нейтральный ответ без статусов `аккаунт найден/не найден`;
- после обработки запроса поле `Логин или email` очищается;
- текст результата заменён на короткое нейтральное сообщение.
